(function ($) {
    ...
    document.addEventListener('readystatechange', event => {
        if (event.target.readyState === "complete") {
            ...
            const a = new Date().toString().split('(')[0].split(':')

            let last = Number(a[0].substr(-2))
            if (last == 23) {
                last = '00'
                let d = Number(a[0].split(' ')[2])
                let newd = d + 1
                newd = newd.toString()
                a[0] = a[0].replaceAt(8, newd.substr(0, 1)).replaceAt(9, newd.substr(1))
            }
            else {
                last += 1
                last = last.toString()
            ...
            let f = a[0].replaceAt(a[0].length - 1, last.substr(1)).replaceAt(a[0].length - 2, last.substr(0, 1))
            let s = a[1]

            let out = new Date(f + ':' + s)
            console.log(out)
            for (var i = 0; i < clockdiv.length; i++) {
                countDownDate[i] = new Array();
                countDownDate[i]['el'] = clockdiv[i];
                countDownDate[i]['time'] = out.getTime();
